...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    ...
    WHERE o.customer_id = v_customer_uuid
  ...
  VALUES (
    ...
    p_landmark,
    p_pincode,
    'NM-' || to_char(now(), 'YYYYMMDDHH24MISSMS') || '-' || substr(gen_random_uuid()::text, 1, 6),
    v_subtotal,
    v_total,
    v_total,
    p_payment_method,
  ...
  LOOP
    v_product_id := NULLIF((v_item ->> 'product_id')::text, '')::bigint;
    v_quantity := NULLIF((v_item ->> 'quantity')::text, '')::integer;
    SELECT p.stock INTO v_previous_stock FROM public.products p WHERE p.id = v_product_id FOR UPDATE;
