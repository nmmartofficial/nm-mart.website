  ...
  return (
    <section className="py-12 bg-white/5 rounded-[3rem] border border-white/10 m-4 shadow-2xl">
      <div className="container mx-auto px-6">
        <h3 className="text-2xl font-black mb-6 italic text-[#FFD700] text-center uppercase tracking-tighter">NM MART PRICE CHECKER</h3>
        
        <form onSubmit={handleSearch} className="relative max-w-2xl mx-auto mb-10">
          <input 
            type="text" 
            placeholder="Product ka naam (Lux, Surf, etc...)" 
            value={query} 
        ...
        <div className="grid gap-4 max-w-4xl mx-auto">
          {results.length > 0 ? results.map((item, index) => (
            ...
            <p className="opacity-30 italic text-white text-center">Nahi mila? Sahi spelling likhein...</p>
          )}
        </div>
      </div>
    </section>
