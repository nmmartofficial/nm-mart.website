...
import { Search, ShoppingCart, Send, X, Plus, Minus, Loader2, Tag, Percent, Flame } from "lucide-react";
...
const ProductSearch = () => {
  ...
  const addToCart = (product: any) => {
    ...
    const existing = cart.find((item) => item["Item Name"] === name);
    if (existing) {
      setCart(cart.map((item) => (item["Item Name"] === name ? { ...item, qty: item.qty + 1 } : item)));
    } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 font-sans text-gray-900">
      <div className="bg-white p-6 shadow-sm sticky top-0 z-40">
        <div className="max-w-6xl mx-auto flex flex-col gap-4 text-left">
          <div className="flex justify-between items-center">
            <h1 className="text-2xl font-black text-blue-900 italic tracking-tighter uppercase">NM MART</h1>
            <div className="bg-red-600 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase italic animate-pulse">
               MEGA 50% OFF ZONE 🔥
            </div>
          </div>
      ...
      {!loading && (
        <div className="bg-white py-3 shadow-sm overflow-x-auto whitespace-nowrap px-4 border-b border-gray-100 sticky top-[136px] z-30">
          <div className="max-w-6xl mx-auto flex gap-2">
            <button
              onClick={() => setSelectedCategory("All")}
              className={`px-6 py-2 rounded-xl text-[11px] font-black uppercase flex items-center gap-2 transition-all ${
                selectedCategory === "All" ? "bg-red-600 text-white shadow-lg" : "bg-gray-100 text-gray-400"
              }`}
            >
              <Flame size={14}/> Top Offers
            </button>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filtered.map((item, idx) => {
              ...
              return (
                <div key={idx} className="bg-white rounded-[2rem] shadow-sm border border-gray-100 flex flex-col hover:shadow-xl transition-all relative overflow-hidden group">
                  {discount > 0 && (
                    <div className="absolute top-3 left-3 bg-red-600 text-white text-[10px] font-black px-2 py-1 rounded-lg z-10 flex items-center gap-1">
                  ...
                  <div className="h-40 bg-white flex items-center justify-center p-4">
                    {item["Barcode"] && item["Barcode"].length > 5 ? (
                      ...
                      <div className="w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center text-blue-300 font-black text-2xl uppercase">
                  ...
                  <div className="p-4 pt-0 flex-grow flex flex-col text-left">
                    ...
                    <h3 className="font-bold text-gray-800 text-[11px] uppercase h-8 overflow-hidden leading-tight mb-2">{item["Item Name"]}</h3>
                    ...
                    <button onClick={() => addToCart(item)} className="w-full bg-[#25D366] text-white py-3 rounded-2xl font-black text-[10px] uppercase flex items-center justify-center gap-2 shadow-md">
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md z-50">
          <button onClick={() => setIsCartOpen(true)} className="w-full bg-blue-600 text-white p-5 rounded-[2rem] font-black text-lg flex justify-between items-center shadow-2xl">
            <div className="flex items-center gap-3 text-left">
              ...
              <span className="text-sm italic">₹{totalBill}</span>
            </div>
            {totalSavings > 0 && <span className="text-[10px] bg-blue-500 px-3 py-1 rounded-full uppercase">Save ₹{totalSavings}</span>}
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[60] flex justify-end">
          <div className="bg-white w-full max-w-md h-full p-8 flex flex-col animate-in slide-in-from-right">
            <div className="flex justify-between items-center mb-8 border-b pb-4">
              <h2 className="text-xl font-black text-blue-900 italic uppercase">Shopping Cart</h2>
              <button onClick={() => setIsCartOpen(false)} className="bg-gray-100 p-2 rounded-full"><X/></button>
            </div>
            <div className="flex-grow overflow-y-auto space-y-4">
              {cart.map((item, i) => (
                <div key={i} className="flex justify-between items-center bg-gray-50 p-4 rounded-3xl">
            ...
            <div className="pt-8 mt-6">
              <div className="flex justify-between text-2xl font-black text-blue-900 mb-6 italic border-t pt-4">
                <span className="text-[10px] text-gray-400 not-italic uppercase self-center">Grand Total</span>
                <span>₹{totalBill}</span>
              </div>
              <button 
                onClick={() => {
                ...
                className="w-full bg-[#25D366] text-white py-5 rounded-[2.5rem] font-black text-lg shadow-xl shadow-green-500/20 flex items-center justify-center gap-3"
              >
